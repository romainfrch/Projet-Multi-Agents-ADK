...
def _classify_user_intent(user_text: str) -> str:
    """Classify current turn intent using deterministic keyword rules."""
...
def root_before_agent(
    ...
    **kwargs: Any,
    ...
    if ctx is not None:
        ...
        # Universal fallback for unsupported/noisy inputs.
        if intent == "unknown":
            return genai_types.Content(
                role="assistant",
...
root_agent = LlmAgent(
    ...
    instruction=(
        ...
        "Routage strict:\n"
        "- si forced_intent=quiz -> transfer_to_agent quiz_workflow\n"
    ...
    sub_agents=[fiche_workflow, quiz_workflow],
